...
function handleOrderAction(event) {
  ...
  const orderIdEle = orderItem.querySelector(".order-actions");
  const orderId = orderIdEle.dataset.productid;
...
function showOrderDetails(orderId) {
  window.location.href = `order?productId=${orderId}`;
}
...
function filterOrders(filters) {
  ...
  orderItems.forEach((item) => {
    ...
    if (filters.customerName) {
      ...
      if (
        !customerName.toLowerCase().includes(filters.customerName.toLowerCase())
      ) {
        shouldShow = false;
      }
    }
    ...
    if (filters.shippingUnit) {
      const shippingInfo = item.querySelector(
        ".shipping-info .text-muted"
      ).textContent;
      if (
        !shippingInfo.toLowerCase().includes(filters.shippingUnit.toLowerCase())
      ) {
        shouldShow = false;
      }
    }
  ...
  if (countElement) {
    if (count === null) {
      const visibleOrders = document.querySelectorAll(
        '.order-item:not([style*="display: none"])'
      );
...
function updatePagination(currentPage = 1, totalPages = 95, totalItems = 2373) {
  ...
  prevItem.innerHTML = `<a class="page-link" href="#" tabindex="${
    currentPage === 1 ? "-1" : "0"
  }" ${currentPage === 1 ? 'aria-disabled="true"' : ""}>Trước</a>`;
  ...
  nextItem.innerHTML = `<a class="page-link" href="#" ${
